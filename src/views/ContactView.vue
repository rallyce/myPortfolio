<template>
    <section class="min-h-screen w-full flex flex-col" ref="contactSection" id="contact-section">
        ...
        <div :class="['flex flex-col my-auto transition-all motion-reduce:transition-none duration-500 delay-300', visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm']">
            ...
            <div id="work1" v-else class="space-y-8 mx-auto">
                <h1 class="text-3xl font-extrabold text-center">Get in Touch</h1>
                <p id="work1" v-for="msg in content.externalLink.note" class="text-slate-500 dark:text-slate-300">{{ msg }}<br/></p>
                <div class="flex flex-col sm:flex-row items-center justify-center space-y-5 sm:space-x-5">
                    <template v-if="content.externalLink.link.email">
                        <a :href="'mailto:'+content.externalLink.link.email">
                            <button class="py-2 px-6 bg-transparent border border-button-color shadow-sm shadow-button-color hover:text-white hover:bg-button-color duration-300 focus:bg-button-color focus:text-white active:bg-button-color active:text-white">Send an Email</button>
                        </a>
                        <p id="work1" class="text-sm mt-8 sm:mt-0 text-slate-500 dark:text-slate-300">{{ content.externalLink.responseTimeMessage }}</p>
                    </template>
                    <template v-else>
