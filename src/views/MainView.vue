<template>
	<SplashView v-if="showingSplash"/>
  	...
  	<div v-show="!showingSplash" id="main-view" class="min-h-screen overscroll-contain dark:text-slate-300 text-base md:text-xl">
			...
			<div class="flex flex-col md:grid md:grid-cols-6 min-h-full">
					...
					<ul class="md:fixed md:bottom-0 mb-7 flex items-center space-x-8 md:flex-col md:space-y-4 md:space-x-0">
						<li v-show="githubLink" :class="['transition-all motion-reduce:transition-none duration-500', showLanding ? 'translate-y-0 opacity-1' : '-translate-y-4 opacity-0']">
							<a :href="githubLink" target="_blank">
								<font-awesome-icon id="work1" icon="fa-brands fa-github" class="h-8 w-8 md:h-12 md:w-12 text-black transition ease-out hover:-translate-y-1 motion-reduce:hover:translate-y-0 duration-300 dark:text-slate-300"></font-awesome-icon>
							</a>
						</li>
						<li v-show="linkedinLink" :class="['transition-all motion-reduce:transition-none duration-500 delay-[100ms]', showLanding ? 'translate-y-0 opacity-1' : '-translate-y-4 opacity-0']">
							<a :href="linkedinLink" target="_blank">
								<font-awesome-icon id="work1" icon="fa-brands fa-linkedin" class="h-8 w-8 md:h-12 md:w-12 text-black transition ease-out hover:-translate-y-1 motion-reduce:hover:translate-y-0 duration-300 dark:text-slate-300"></font-awesome-icon>
							</a>
						</li>
				...
				<div class="col-span-4 flex-1 flex-col order-1 md:order-none">
					<div class="container mx-auto max-w-full 2xl:max-w-6xl">
						...
						<footer id="work1" class="hidden text-center md:block mb-5">
							<p>Designed by Omar Zuniga Calderon</p>
							<p>Based and created by Maxim Shelepov</p>
						</footer>
					</div>
				</div>
				<footer id="work1" class="block text-center text-sm md:hidden order-last mb-5">
					<p>Designed by Omar Zuniga Calderon</p>
					<p>Based and created by Maxim Shelepov</p>
				</footer>
			</div>
